...
#pragma message(__LOC__"Check RLC-AM and correct this.")
		rlcMode = LTENR_RLCMODE_UM;
		break;
	}
...
static ptrLTENR_SDAP_DelayEntity LTENR_SDAP_DELAY_FINDENTITY(ptrLTENR_SDAP_DelayEntity t) {
	while (t) {
		if (t->TxTime == pstruEventDetails->pPacket->pstruMacData->dArrivalTime)
			return t;
...
void fn_NetSim_LTENR_SDAP_CalcQOS() {
	...
	if (!SDAPentity) {
		fnNetSimError(" SDAP Entity not Found");
	}
	else {
		SDAPentity->countIn++;
