...
{
	TDMA_NODE_MAC* mac=NULL;
	NETSIM_ID loop;
	for (loop = 0; loop < NETWORK->nDeviceCount; loop++)
	{
		if (DEVICE_MACLAYER(loop + 1, 1)->nMacProtocolId != MAC_PROTOCOL_TDMA)
			continue;

		mac = (TDMA_NODE_MAC*)DEVICE_MACVAR(loop + 1, 1);
		break;
	}
	for(loop=0;loop<NETWORK->nDeviceCount;loop++)
	{
...
{
	if (DEVICE_MACLAYER(nNodeId, 1)->nMacProtocolId != MAC_PROTOCOL_TDMA)
		return 1;
