	...
	case PHYSICAL_LAYER:
		{
			...
			fn_NetSim_Config_read_frequency(&phy->frequency,
											xmlNetSimNode,
											ALOHA_FREQUENCY_DEFAULT);
			fn_NetSim_Config_read_bandwidth(&phy->bandwidth, 
											xmlNetSimNode,
											ALOHA_BANDWIDTH_DEFAULT);
			fn_NetSim_Config_read_txPower(&phy->tx_power,
										  xmlNetSimNode,
										  ALOHA_TX_POWER_DEFAULT);
			...
			free(szVal);
			fn_NetSim_Config_read_dataRate(&phy->data_rate,
										   xmlNetSimNode,
										   ALOHA_DATA_RATE_DEFAULT);
		}
		break;
	case MAC_LAYER:
		{
