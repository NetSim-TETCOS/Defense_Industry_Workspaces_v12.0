...
static void LTENR_MACSchedular_UpdateInfoForUE(ptrLTENR_GNBMAC mac, ptrLTENR_GNBPHY phy,
											   ...
											   ptrLTENR_UESCHEDULARINFO info, bool isUplink)
{
	ptrLTENR_AMCINFO amc = isUplink ? assocInfo->uplinkAMCInfo : assocInfo->downlinkAMCInfo;
	info->cqiIndex = amc->cqiTable.CQIIndex;
	info->bitsPerPRB = LTENR_calculateTBSSize(phy, 1, isUplink, amc->mcsTable);
	if (isUplink)
...
static void LTENR_MAC_AddSchedularStausForUE(ptrLTENR_GNBMAC mac, ptrLTENR_GNBPHY phy,
											 ptrLTENR_ASSOCIATEDUEPHYINFO assocInfo, ptrLTENR_SCHEDULARINFO si)
{
	...
	LTENR_MACSchedular_UpdateInfoForUE(mac, phy, assocInfo, uinfo, true);
	LTENR_MACSchedular_UpdateInfoForUE(mac, phy, assocInfo, dinfo, false);
} 
...
static void LTENR_MAC_InitSchedularStatus(ptrLTENR_GNBMAC mac)
{
	...
	if (mac->schedularInfo)
	{
		info = mac->schedularInfo;
		info->slotType = phy->frameInfo->slotType;
	...
	else
	{
		...
		mac->schedularInfo = info;
		info->gnbId = mac->gnbId;
		info->gnbIf = mac->gnbIf;
		info->PRBCount = sc->PRBCount;
		LTENR_PHY_GET_OH(sc, &info->OH_DL, &info->OH_UL);
		info->slotType = phy->frameInfo->slotType;
		info->symbolPerSlot = sc->symbolPerSlot;
	...
	while (assocInfo)
	{
		LTENR_MAC_AddSchedularStausForUE(mac, phy, assocInfo, info);
...
{
	ptrLTENR_SCHEDULARINFO si = mac->schedularInfo;
...
void LTENR_NotifyMACForStartingSlot()
{
	...
	//schedular call
	LTENR_PRB_Schedular(mac->schedularInfo);
